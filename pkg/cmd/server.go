...
func init() {
	...
	ServerCmd.Flags().Int("log-output-retention", 2000, "Maximum log rows per run to retain.")
	ServerCmd.Flags().Duration("log-output-max", 7*24*time.Hour, "Run logs store period")
