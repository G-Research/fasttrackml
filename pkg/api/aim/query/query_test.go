...
func (s *QueryTestSuite) TestPostgresDialector_Ok() {
	tests := []struct {
		...
		},
		{
			...
			dialector:    postgres.Dialector{}.Name(),
			expectedSQL:  `SELECT * FROM "runs" WHERE ("runs"."name" ~ 'run' AND "runs"."lifecycle_stage" <> $1) ORDER BY "runs"."run_uuid" LIMIT 1`,
			expectedVars: []interface{}{models.LifecycleStageDeleted},
		},
	}
...
func (s *QueryTestSuite) TestSqliteDialector_Ok() {
	tests := []struct {
		...
		},
		{
			...
			expectedSQL:  `SELECT * FROM "runs" WHERE ("runs"."name" regexp 'run' AND "runs"."lifecycle_stage" <> $1) ORDER BY "runs"."run_uuid" LIMIT 1`,
			expectedVars: []interface{}{models.LifecycleStageDeleted},
		},
	}
