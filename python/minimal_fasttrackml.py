...
def main():
    ...
    metrics = []
    for i in range(1000):
        metrics.append(Metric("loss", random(), timestamp, i, context={"context_key": "context_value3"}))
        metrics.append(Metric("precision", random(), timestamp, i, context={"context_key": "context_value6"}))

    start_time = time.time()
    result = client.log_batch(run_id, metrics=metrics, synchronous=False)
    end_time = time.time()
    print(f"Batch logging took {(end_time - start_time)*1000} milliseconds")
