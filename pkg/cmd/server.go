...
func init() {
	...
	ServerCmd.Flags().Int("run-log-output-max-retain-number", 2000, "Maximum log rows per run to retain.")
	ServerCmd.Flags().Duration("run-log-output-store-period", 1*time.Minute, "Run logs store period")
