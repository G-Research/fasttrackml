...
from mlflow.tracking.fluent import search_experiments, get_experiment_by_name, _get_experiment_id, NUM_RUNS_PER_PAGE_PANDAS, SEARCH_MAX_RESULTS_PANDAS
...
def get_metric_histories(
    ...
    def pagination_wrapper_func(number_to_get, next_page_token):
        ...
        return PagedList(result["runs"], result.get("next_page_token"))
    ...
    secondary = "step" if index == "timestamp" else "timestamp"
    multi_index = []
    data = {
        secondary: [],
        "value": [],
    ...
    for run in runs:
        ...
        for metric in run["data"]["metrics"]:
            multi_index.append((id, metric["key"], metric[index]))
            data[secondary].append(metric[secondary])
            data["value"].append(metric["value"])

    return pandas.DataFrame(data, index=pandas.MultiIndex.from_tuples(multi_index, names=("run_id", "key", index)))
